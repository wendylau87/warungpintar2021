...
import (
	...
	itemuc "github.com/wendylau87/warungpintar2021/mastersvc/usecases/item"
...
func TestMain(t *testing.M){
	...
	if err != nil{
		...
		uc = itemuc.InitItemUsecase(*log, dom)
	...
	Convey("create Item", t, func() {
		...
		for _, tc := range testCases {
			Convey(fmt.Sprintf("%d - [%s] : %s", tc.testID, tc.testType, tc.testDesc), func() {
				_, err := uc.CreateItem(tc.payload)
				if tc.testType == "P" {
					So(err, ShouldBeNil)
